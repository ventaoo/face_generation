...
class Generator_conditional(nn.Module):
    def __init__(self, latent_dim=100, img_channels=3, num_classes=2, embedding_dim=28):
        super().__init__()
        self.init_size = 4  # 初始特征图尺寸
        ...
        self.fc = nn.Sequential(
            nn.Linear(latent_dim + embedding_dim, 1024 * self.init_size ** 2),
            nn.BatchNorm1d(1024 * self.init_size ** 2),
            nn.LeakyReLU(0.2)
        )
    ...
    def forward(self, z, labels):
        ...
        out = self.fc(z)
        out = out.view(out.shape[0], 1024, self.init_size, self.init_size) # [batch, channel, 4, 4]
...
if __name__ == '__main__':
    ...
    print(G_conditional(z, labels).shape)